...
trait BiViaSourceOps {
  implicit def biViaSourceOps[L, R](source: Graph[SourceShape[Either[L, R]], akka.NotUsed]): BiViaSource[L, R] =
    new BiViaSource(source)
}

object BiViaSourceOps extends BiViaSourceOps

class BiViaSource[L, R](val source: Graph[SourceShape[Either[L, R]], akka.NotUsed]) extends AnyVal {
  /** Process each side of an Either through a separate Flows and then combine the result back into an Either
   *  Does not preserve order
   */
  def biVia[L1, R1](left: Graph[FlowShape[L, L1], akka.NotUsed], right: Graph[FlowShape[R, R1], akka.NotUsed]): Source[Either[L1, R1], akka.NotUsed] =
